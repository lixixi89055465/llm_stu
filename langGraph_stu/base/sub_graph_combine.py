...
def format_report_for_slack(state):
    ...
    format_report = 'foo bar'  # 固定的格式化报告内容
    return {'report': format_report}  # 返回包含格式化
...
question_answer_feedback = Logs(
    id='2',
    question='如何使用Chroma 向量存储 ？',
    answer='要使用 Chroma ，请定义：rag_chain=create_retrieval_chain(retriever,question_answer) ',
    grade=0,
    grader='文档相似性回顾',
    feedback='检索到的文档一般讨论了向量纯属，但没有专门讨论Chroma'
)
