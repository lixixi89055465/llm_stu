'''
https://www.bilibili.com/video/BV12iQBYWEx9?t=1854.7&p=9
...
def configure_retriever(uploaded_files):
    # 读取上传的文档，并写入一个临时目录
    docs = []
    temp_dir = tempfile.TemporaryFile(dir=r'D:\\')
...
# 如果session_state 中没有消息记录或用户点击了清空聊天记录按钮，则初始化消息记录
if 'message' not in st.session_state or st.sidebar.button('清空聊天记录'):
    st.session_state['messages'] = [{'role': 'assistant', 'content': '您好，我是文档文档助手'}]
# 加载历史聊天记录
...
# 指令模板
instructions = '''
您是一个设计用于査询文档来回答问题的代理。您可以使用文档检索工具，并基于检索内容来回答问题您可能不查询文档就知道答案，
但是您仍然应该查询文档来获得答案。
...
Action Input: {input}
0bservation:the result of the action
[ZWJ]```
...
if user_query:
    ...
    with st.chat_message('assistant'):
        ...
        # agent 执行过程日志回调显示在Streamlit Container(如思考、选择工具、执行查询、查询结果等)
