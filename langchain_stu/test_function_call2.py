...
llm = Ollama(base_url=base_url, model="qwen2:latest", temperature=0.7)
...
#     keyword = '充电站'
#     search_around(keyword=keyword, location=location)
function_ollama = OllamaFunctions(base_url=base_url, model="qwen2:latest")
    ...
    for def_name in def_names:
        ...
        if function_action:
            ...
            if 'end' not in result:
                ...
                res = llm_tools.invoke(json.dumps(messages, ensure_ascii=False))
                def_names = json.loads(res)['tool_calls']
            else:
                def_names = None
